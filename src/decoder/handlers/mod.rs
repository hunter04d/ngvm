...
fn decode_three_stack_ref(code: Opcode, chunk: &Chunk) -> DecodeResult {
    let res_ref = chunk.read_ref_stack(0).unwrap();
    let op1_ref = chunk.read_ref_stack(1).unwrap();
    let op2_ref = chunk.read_ref_stack(2).unwrap();
    let repr = format!(
        "{:?} <res>{} <op1>{} <op2>{}",
...
pub(super) fn decode_i_neg(chunk: &Chunk) -> DecodeResult {
    let stack_ref = chunk.read_ref_stack(0).unwrap();
