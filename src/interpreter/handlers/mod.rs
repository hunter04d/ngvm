...
pub(super) fn handle_trace_stack_value(chunk: &Chunk, vm: &mut Vm) -> InterpreterResult {
    let error = run(|| {
        let stack_ref = chunk.read_ref_stack(0).ok_or(VmError::InvalidBytecode)?;
    ...
    InterpreterResult::new(1 + refs_size(1)).with_error_opt(error.err())
}
...
pub(crate) fn noop(chunk: &Chunk, _vm: &mut Vm) -> InterpreterResult {
    panic!("a bad opcode detected <{}>", chunk.single());
}
